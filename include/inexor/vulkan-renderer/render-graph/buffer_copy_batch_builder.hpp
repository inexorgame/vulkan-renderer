...
private:
    ...
    struct BatchValue {
        ...
        VkDeviceSize min_offset{std::numeric_limits<VkDeviceSize>::max()};
        VkDeviceSize max_end{0};
    };

    std::unordered_map<BatchKey, BatchValue, BatchKeyHash> m_batches;
    std::vector<VkBufferCopy> m_scratch_regions;
...
public:
    BufferCopyBatchBuilder() = default;
    ...
    void add(const PendingBufferCopy &copy_request);
    void add(std::span<const PendingBufferCopy> copy_requests);
