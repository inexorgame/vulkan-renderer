...
private:
    uint32_t available_instance_extensions = 0;

    uint32_t available_instance_layers = 0;

    uint32_t available_device_layers = 0;

    uint32_t available_device_extensions = 0;
...
public:
    ...
    bool has_instance_layer(const std::string &instance_layer_name);
    ...
    bool has_device_layer(const VkPhysicalDevice &graphics_card, const std::string &device_layer_name);
    ...
    /// @note Device layers and device extensions are coupled to a certain graphics card which needs to be specified as parameter.
    bool has_device_extension(const VkPhysicalDevice &graphics_card, const std::string &device_extension_name);
    ...
    bool has_presentation(const VkPhysicalDevice &graphics_card, const VkSurfaceKHR &surface);

    /// @brief Checks if swapchain is available for a certain graphics card.
    /// @param graphics_card [in] The selected graphics card.
    /// @return true if swapchain is available, false otherwise.
    bool has_swapchain(const VkPhysicalDevice &graphics_card);
};
