...
/// This is the base class for e.g. uniform buffers or vertex/index buffers.
/// @note The core of Inexor's memory management is Vulkan Memory Allocator library.
class GPUMemoryBuffer {
protected:
    std::string m_name;
...
public:
    /// @brief Constructs the GPU memory buffer without specifying the actual data to fill in, only the memory size.
    /// @param device [in] The const reference to a device RAII wrapper instance.
    /// @param name [in] The internal debug marker name of the GPU memory buffer.
    /// @param buffer_size [in] The size of the memory buffer in bytes.
    /// @param buffer_usage [in] The buffer usage flags.
    /// @param memory_usage [in] The VMA memory usage flags which specify the required memory allocation.
    GPUMemoryBuffer(const Device &device, const std::string &name, const VkDeviceSize &buffer_size,
                    const VkBufferUsageFlags &buffer_usage, const VmaMemoryUsage &memory_usage);

    /// @brief Constructs the GPU memory buffer and specifies the actual data to fill it in.
    /// @param device [in] The const reference to a device RAII wrapper instance.
    /// @param name [in] The internal debug marker name of the GPU memory buffer.
    /// @param buffer_size [in] The size of the memory buffer in bytes.
    /// @param data [in] A pointer to the data to fill the GPU memory buffer with.
    /// @param data_size [in] The size of the memory to copy from data pointer.
    /// @param buffer_usage [in] The buffer usage flags.
    /// @param memory_usage [in] The VMA memory usage flags which specify the required memory allocation.
