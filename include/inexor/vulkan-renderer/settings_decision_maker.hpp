...
class VulkanSettingsDecisionMaker {
    ...
    /// @param graphics_card [in] The graphics card which will be rated.
...
public:
    ...
    /// @param graphics_card [in] The graphics card which will be checked for suitability.
    ...
    /// @note Please take a look at Inexor's advanced device selection mechanism which is build into this method.
    /// @note If there is only one graphics card available, we don't have a choice and must try that one.
    /// @note The user can manually specify which graphics card will be used with command line argument --gpu <index>.
    /// Please note that the graphics cards index starts with 0.
    /// @param vulkan_instance [in] A pointer to the Vulkan instance handle.
    /// @param preferred_graphics_card_index [in] The preferred graphics card (by array index).
    ...
    /// @param surface [in] The selected (window) surface.
    ...
    /// @brief Automatically finds the image transform, relative to the presentation engine's natural orientation,
    ...
    /// @param graphics_card [in] The selected graphics card.
    /// @param surface [in] The selected (window) surface.
    /// @param vsync [in] True if vertical synchronization is desired, false otherwise.
    ...
    /// @brief Tries to find a queue family which supports VK_QUEUE_TRANSFER_BIT.
    ...
    /// @param graphics_card [in] The selected graphics card.
    /// @param formats [in] The depth buffer formats to check for.
    /// @param tiling [in] The desired depth buffer's image tiling.
    /// @param feature_flags [in] The desired depth buffer's feature flags.
