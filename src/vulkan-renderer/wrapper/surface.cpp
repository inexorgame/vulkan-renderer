...
Surface::Surface(const wrapper::Instance &instance, GLFWwindow *window) : m_instance(instance), m_window(window) {
    // NOTE: glfwCreateWindowSurface does return a VkResult, so the VulkanException will work correctly
    if (const auto result = glfwCreateWindowSurface(m_instance.m_instance, m_window, nullptr, &m_surface);
        result != VK_SUCCESS) {
