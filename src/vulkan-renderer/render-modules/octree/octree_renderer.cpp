...
OctreeRenderer::OctreeRenderer(std::shared_ptr<RenderGraph> render_graph, std::weak_ptr<Swapchain> swapchain,
    ...
    m_mvp_matrix = render_graph->add_buffer(
        ...
        [&]() {
            ...
            // BufferUpdateBuilder, which is given to the lambda as a parameter. Maybe this can even help us to simplify
            // the pipeline barrier placement!
    ...
    m_octree_pass = render_graph->add_graphics_pass([&](GraphicsPassBuilder &pass_builder) {
        return pass_builder
            .conditionally_writes_to(m_color_buffer, !m_color_buffer.expired(), VkClearValue{0.0f, 0.0f, 0.0f})
