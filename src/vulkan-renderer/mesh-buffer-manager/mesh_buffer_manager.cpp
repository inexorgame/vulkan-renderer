...
namespace vulkan_renderer {
	...
	VkResult InexorMeshBufferManager::initialise(const VkDevice& device, const std::shared_ptr<VulkanDebugMarkerManager> debug_marker_manager, const VmaAllocator& vma_allocator, const uint32_t data_transfer_queue_family_index, const VkQueue& data_transfer_queue)
	{
		assert(device);
														   ...
														   std::shared_ptr<InexorMeshBuffer>& output_mesh_buffer)
	{
		...
		result = create_buffer(internal_mesh_buffer_name, vertex_buffer, vertex_buffer_size, VK_BUFFER_USAGE_TRANSFER_DST_BIT|VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, VMA_MEMORY_USAGE_CPU_ONLY);
		if(VK_SUCCESS != result)
		{
			vulkan_error_check(result);
			return result;
																			 ...
																			 std::shared_ptr<InexorMeshBuffer>& mesh_buffer_output)
	{
		...
		VkResult result = create_buffer(staging_vertex_buffer_name, staging_vertex_buffer, vertex_buffer_size, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VMA_MEMORY_USAGE_CPU_ONLY);
		if(VK_SUCCESS != result)
		{
			vulkan_error_check(result);
			return result;
		...
		result = create_buffer(vertex_buffer_name, vertex_buffer, vertex_buffer_size, VK_BUFFER_USAGE_TRANSFER_DST_BIT|VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, VMA_MEMORY_USAGE_CPU_ONLY);
		if(VK_SUCCESS != result)
		{
			vulkan_error_check(result);
			return result;
		...
		result = create_buffer(internal_mesh_buffer_name, index_buffer, index_buffer_size, VK_BUFFER_USAGE_TRANSFER_DST_BIT|VK_BUFFER_USAGE_INDEX_BUFFER_BIT, VMA_MEMORY_USAGE_CPU_ONLY);
		if(VK_SUCCESS != result)
		{
			vulkan_error_check(result);
			return result;
