...
ImGUIOverlay::ImGUIOverlay(const wrapper::Device &device, render_graph::RenderGraph &render_graph,
                           const std::weak_ptr<render_graph::TextureResource> back_buffer,
                           const std::weak_ptr<render_graph::TextureResource> depth_buffer,
    ...
    m_vertex_buffer =
        render_graph.add_buffer("ImGui", BufferType::VERTEX_BUFFER, DescriptorSetUpdateFrequency::PER_FRAME, [&]() {
            ...
            m_vertex_buffer.lock()->enqueue_update(&m_vertex_data, sizeof(m_vertex_data));
            m_index_buffer.lock()->enqueue_update(&m_index_data, sizeof(m_index_data));
        });
