    ...
    if (node.mesh) {
        for (const auto &primitive : node.mesh->primitives) {
            if (primitive.material.alpha_mode == alpha_mode) {
                ...
                cmd_buf.bind_descriptors(descriptor_sets, pipeline_layout);
...
void ModelPbrRenderer::setup_stage(RenderGraph *render_graph, const TextureResource *back_buffer,
    ...
    spdlog::warn("You forgot to fill me out :D");
    ...
    auto gltf_stage = render_graph->add<GraphicsStage>("gltf2 model")
                          ...
                          ->set_on_record([&](const PhysicalStage &physical, const wrapper::CommandBuffer &cmd_buf) {
                              render_model(model.nodes(), scene_descriptor_set, cmd_buf, physical.pipeline_layout());
                          });
