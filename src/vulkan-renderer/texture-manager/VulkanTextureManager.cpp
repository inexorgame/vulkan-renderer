...
namespace vulkan_renderer {
	...
	VkResult VulkanTextureManager::initialise(const VkDevice& device, const VkPhysicalDevice& graphics_card, const std::shared_ptr<VulkanDebugMarkerManager> debug_marker_manager_instance,  const VmaAllocator& vma_allocator, const uint32_t& transfer_queue_family_index, const VkQueue& data_transfer_queue)
	{
		assert(device);
		...
		dbg_marker_manager->set_object_name(device, (uint64_t)(data_transfer_command_pool), VK_DEBUG_REPORT_OBJECT_TYPE_COMMAND_POOL_EXT, "Command buffer for texture data transfer.");

		return result;
	}


	VkResult VulkanTextureManager::create_texture_buffer(InexorBuffer& buffer_object, const VkBufferUsageFlags& buffer_usage, const VmaMemoryUsage& memory_usage)
	{
		assert(vma_allocator_handle);
		assert(dbg_marker_manager);
		
		spdlog::debug("Creating a texture data buffer.");
	...
	VkResult VulkanTextureManager::create_texture_image(InexorTexture& texture, const uint32_t& texture_width, const uint32_t& texture_height, const VkFormat& format, const VkImageTiling& tiling, const VmaMemoryUsage& memory_usage, const VkBufferUsageFlags& buffer_usage, const VkImageUsageFlags& image_usage_flags)
	{
		...
		VkResult result = vkCreateImage(vulkan_device, &texture_image_create_info, nullptr, &texture.image);
		vulkan_error_check(result);
		...
		/*
		texture.create_info.sType            = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO;
		texture.create_info.size             = texture.size;
		texture.create_info.usage            = buffer_usage;
		texture.create_info.sharingMode      = VK_SHARING_MODE_EXCLUSIVE;
		*/

		texture.allocation_create_info.usage = memory_usage;
		texture.allocation_create_info.flags = 0;
		
		result = vmaCreateImage(vma_allocator_handle, &texture_image_create_info, &texture.allocation_create_info, &texture.image, &texture.allocation, nullptr);
		vulkan_error_check(result);

		// TODO: Debug marker for memory region!
	...
	{
		spdlog::debug("Started recording single time commands.");
		...
		VkResult result = vkAllocateCommandBuffers(vulkan_device, &command_buffer_allocate_info, &data_transfer_command_buffer);
		vulkan_error_check(result);
	...
	{
		spdlog::debug("Ended recording single time commands.");
		...
		vkFreeCommandBuffers(vulkan_device, data_transfer_command_pool, 1, &data_transfer_command_buffer);

		return VK_SUCCESS;
	}
	
	
	VkResult VulkanTextureManager::create_texture_image_view(InexorTexture& texture, const VkFormat& format)
	{
		...
		VkResult result = vkCreateImageView(vulkan_device, &view_info, nullptr, &texture.view);
		vulkan_error_check(result);
	...
	VkResult VulkanTextureManager::create_texture_from_file(const std::string& file_name, InexorTexture& texture)
	{
		int texture_width = 0;
		...
		VkResult result = create_texture_buffer(staging_buffer_for_texture, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VMA_MEMORY_USAGE_CPU_TO_GPU);
		vulkan_error_check(result);
		...
		vmaDestroyBuffer(vma_allocator_handle, staging_buffer_for_texture.buffer, texture.allocation);

		// Free memory of the texture's staging buffer.
		vmaFreeMemory(vma_allocator_handle, staging_buffer_for_texture.allocation);
		...
		list_of_textures.push_back(texture);

		return VK_SUCCESS;
	}

	
	VkResult VulkanTextureManager::create_texture_sampler(InexorTexture& texture)
	{
		...
		sampler_create_info.compareOp     = VK_COMPARE_OP_NEVER; // TODO?
		...
		vkGetPhysicalDeviceProperties(physical_device, &graphics_card_properties);
		...
		*/
		...
		vkCreateSampler(vulkan_device, &sampler_create_info, nullptr, &texture.sampler);
	...
	VkImageView VulkanTextureManager::get_texture_view(const uint32_t index)
	{
		return list_of_textures[index].view;
	}
    
	
	VkSampler VulkanTextureManager::get_texture_sampler(const uint32_t index)
	{
		return list_of_textures[index].sampler;
	}


	void VulkanTextureManager::shutdown_textures()
	{
		for(std::size_t i=0; i<list_of_textures.size(); i++)
		{
			// Every texture object is responsible for its own resources.
			list_of_textures[i].destroy_texture(vulkan_device, vma_allocator_handle);
		}
	}
