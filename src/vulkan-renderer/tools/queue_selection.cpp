...
QueueFamilyIndexCandidates determine_queue_family_indices(const std::vector<VkQueueFamilyProperties> &props,
    ...
    if (!compute_candidate) {
        ...
        compute_candidate =
            find_queue_family_index_if([&](const std::uint32_t index, const VkQueueFamilyProperties &props) {
                ...
                // associated with graphics or only associated with transfer, but not both. However, this should be
    ...
    // At this point, it could be that a distinct compute queue family was found, or a non-distinct compute queue family
    // was found in a second attempt, or no compute queue family was found at all.
    if (return_value.compute) {
        // If a compute queue family weas found that is not distinct, it means that it is of some other type as well.
        // This means the selected queue family for compute could be of type graphics or transfer as well.
        // Vulkan API only allows us to have one distinct entry for every queue family in VkDeviceQueueCreateInfo!
        // We must check if the queue family index for compute is equal to the queue family index for graphics (if not
        // std::nullopt). You might want to take a moment to read and to understand this if statement.
    ...
    // At this point, it could be that a distinct transfer queue was found, or a non-distinct transfer queue was found
    // in a second attempt, or no transfer queue was found at all.
    if (return_value.transfer) {
        // If a transfer queue family was found which is not distinct, it means that it is of some other type as well.
        // This means the selected queue family for transfer could be of type graphics or transfer as well.
        // Vulkan API only allows us to have one distinct entry for every queue family in VkDeviceQueueCreateInfo!
        // We must check if the queue family index for the transfer queue is equal to the queue family index for the
        // graphics queue (if not std::nullopt) and also make check if the queue family index for the transfer queue is
        // equal to the queue family index for the compute queue (if not std::nullopt).
        // You might want to take a moment to read and to understand this if statement.
