...
Inexor is a MIT-licensed open-source project which develops a new 3D octree game engine by combining `modern C++ <https://awesomecpp.com/>`__ with Vulkan `Vulkan API 1.3 <https://www.khronos.org/vulkan/>`__.
...
- Create a modern C++ Vulkan API codebase which can be used in production.
- Invent an advanced octree system similar to cube-engine, but with an arbitrary number of octree layers instead of only one.

**We use good software engineering practices:**

- `Design Patterns <https://refactoring.guru/>`__ in combination with Vulkan API.
- `Resource acquisition is initialization (RAII) Idiom <https://isocpp.github.io/CppCoreGuidelines/CppCoreGuidelines#Rr-raii>`__ for resource management.
...
- Use of the new `C++ standard library <https://en.cppreference.com/w/cpp/header>`__ (C++11, C++14, C++17, and C++20).
- Automatic `unit testing <https://github.com/google/googletest>`__ and `benchmarking <https://github.com/google/benchmark>`__.
- `Static code analysis <https://en.wikipedia.org/wiki/Static_program_analysis>`__ with `clang-tidy <https://clang.llvm.org/extra/clang-tidy/>`__, `clang scan-build <https://clang.llvm.org/docs/analyzer/user-docs/CommandLineUsage.html#scan-build>`__, `cppcheck <https://cppcheck.sourceforge.io/>`__, and `msvc code analysis <https://docs.microsoft.com/en-us/visualstudio/code-quality/?view=vs-2022>`__.
...
- `ImGui <https://github.com/ocornut/imgui>`__ integration.
- Console log and logfiles using `spdlog <https://github.com/gabime/spdlog>`__.
- An automatic gpu selection mechanism (if no preferred gpu is specified).
...
- Integration of `Vulkan Memory Allocator (VMA) <https://github.com/GPUOpen-LibrariesAndSDKs/VulkanMemoryAllocator>`__ and `volk meta-loader <https://github.com/zeux/volk>`__.
