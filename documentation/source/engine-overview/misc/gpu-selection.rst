...
- If a preferred index is specified, the engine will verify if the index is valid and pick the physical device if it is suitable for the requested device level.
- If the physical device specified by the user is not suitable because of technical reasons or does not support the requested device level, automatic selection rules apply.
